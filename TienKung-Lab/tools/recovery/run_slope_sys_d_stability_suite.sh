...
checkpoint="$project_dir/logs/g1_slope_sys_d.pt"
output_dir="$project_dir/tools/recovery/generated/slope_sys_d_stability_2026-09-04"
...
for slope in "${slopes[@]}"; do
  ...
  if report_complete "$output" "$slope"; then
    echo "[slope-sys-d-stability] slope=$slope already complete; skipping"
    continue
  fi
  echo "[slope-sys-d-stability] slope=$slope starting"
  ...
  if [[ $status -ne 0 ]] || ! report_complete "$output" "$slope"; then
    echo "[slope-sys-d-stability] slope=$slope failed; inspect $log"
    exit 1
  fi
  echo "[slope-sys-d-stability] slope=$slope completed"
done

echo "[slope-sys-d-stability] raw suite completed: $output_dir"
