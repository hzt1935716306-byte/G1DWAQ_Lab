...
analyzer="$project_dir/tools/recovery/analyze_slope_nosys_d_stability.py"
checkpoint="$project_dir/logs/g1_slope_nosys_d/2026-09-04_01-41-55/model_9999.pt"
output_dir="$project_dir/tools/recovery/generated/slope_nosys_d_stability_2026-09-04"
report="$project_dir/tools/recovery/SLOPE_NOSYS_D_STABILITY_COMPARISON.md"
...
# Add the model to the exact mixed-command flat benchmark first. Existing
# policies are skipped by that launcher's persisted-report checks.
...
for slope in "${slopes[@]}"; do
  ...
  if report_complete "$output" "$slope"; then
    echo "[slope-stability] slope=$slope already complete; skipping"
    continue
  fi
  echo "[slope-stability] slope=$slope starting"
  ...
  if [[ $status -ne 0 ]] || ! report_complete "$output" "$slope"; then
    echo "[slope-stability] slope=$slope failed; inspect $log"
    exit 1
  fi
  echo "[slope-stability] slope=$slope completed"
done

"$python_bin" "$analyzer" --input_dir "$output_dir" --output "$report"
echo "[slope-stability] report written to $report"
