...
def prepare():
    ...
    blocks=np.linspace(0,len(full)//64-1,32,dtype=int).tolist()
    ...
    spec=dict(evaluation_role='PERFORMANCE_ONLY',formal_statistics_eligible=False,
        candidates=[64,128,256,512,1024,2048],models=['ppo_plain','context_only'],
        ...
        selection='Maximum geometric mean throughput gain across PPO and Context-only; at least 10% gain, else64',
...
def run(args):
    ...
    assert args.num_envs in spec['candidates'] and len(manifest)%args.num_envs==0
    ...
    target=ROOT/args.model/f'n{args.num_envs}';target.mkdir(parents=True,exist_ok=False)
    ...
    identity.update(prepared,evaluation_role='PERFORMANCE_ONLY',formal_statistics_eligible=False,execution_batch_size=args.num_envs,manifest_hash=digest(manifest),trials=len(manifest),harness_sha256=sha256(Path(__file__)))
    ...
    try:
        ...
        controller=PhysicalInterventions(env,identity,p);execution_start=time.monotonic();query_ids=set();query_failure_ids=set();query_counts=0;failures=0;query_profiles={}
        with torch.inference_mode():
            for offset in range(0,len(manifest),args.num_envs):
                selected=manifest[offset:offset+args.num_envs];controller.clear();obs,extra=env.begin_batch(selected)
        ...
        write_json(target/'sampled_replay_audit.json',dict(evaluation_role='PERFORMANCE_ONLY',selected_trial_ids=sorted(selected_ids),selected_count=len(selected_ids),total_count=len(manifest),mismatch_count=0))
        telemetry.finish();samples=read_jsonl(target/'telemetry.jsonl')
        values=lambda k:[x[k] for x in samples if k in x]
        summary=dict(status='PERFORMANCE_ONLY_COMPLETE',formal_statistics_eligible=False,execution_batch_size=args.num_envs,completed=len(done),wall_time_s=time.monotonic()-wall,trial_execution_wall_time_s=elapsed,
            completed_trials_per_min=len(done)*60/elapsed,physics_vector_steps_per_s=steps*4/elapsed,physics_env_steps_per_s=steps*4*args.num_envs/elapsed,
            ...
            validation='LIGHT all records plus deterministic SAMPLED replay; no automatic full replay',sampled_replay_count=len(selected_ids))
...
if __name__=='__main__':
    a=argparse.ArgumentParser();a.add_argument('command',choices=['prepare','run']);a.add_argument('--model',choices=['ppo_plain','context_only']);a.add_argument('--num_envs',type=int);a.add_argument('--device',default='cuda:0');args=a.parse_args()
